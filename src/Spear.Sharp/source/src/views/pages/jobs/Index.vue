    ...
    <el-table v-loading="loading" :data="list">
      <el-table-column type="expand">
        <template slot-scope="scope">
          <el-form inline class="table-expand" label-width="100px">
            <el-form-item label="method">{{
              ["Get", "Post", "Delete", "Put", "Patch"][scope.row.detail.method]
            }}</el-form-item>
            <el-form-item label="url">{{ scope.row.detail.url }}</el-form-item>
            <el-form-item label="data">{{
              scope.row.detail.data
            }}</el-form-item>
          </el-form>
          <triggers
            :job="scope.row"
      ...
      <el-table-column label="任务分组" prop="group" />
      <el-table-column label="任务名称" prop="name" />
      <el-table-column label="任务类型" prop="type">
        <template slot-scope="scope">
          <el-tag
            :type="['', 'warning', 'success', 'danger'][scope.row.type]"
            >{{ scope.row.typeCn }}</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column label="上次执行时间" prop="prevTime">
        <template slot-scope="scope">{{
          scope.row.prevTime | parseTime()
        }}</template>
      </el-table-column>
      <el-table-column label="下次执行时间" prop="nextTime">
        <template slot-scope="scope">{{
